...
func createRootCommand(ctx context.Context, input *Input, version string) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:               "gha [event name to run] [flags]\n\nIf no event name passed, will default to \"on: push\"\nIf actions handles only one event it will be used as default instead of \"on: push\"",
	...
	rootCmd.PersistentFlags().StringVarP(&input.cacheServerPath, "cache-server-path", "", filepath.Join(CacheHomeDir, "ghacache"), "Defines the path where the cache server stores caches.")
	rootCmd.PersistentFlags().StringVarP(&input.cacheServerExternalURL, "cache-server-external-url", "", "", "Defines the external URL for if the cache server is behind a proxy. e.g.: https://gha-cache-server.example.com. Be careful that there is no trailing slash.")
	...
	rootCmd.PersistentFlags().StringVarP(&input.actionCachePath, "action-cache-path", "", filepath.Join(CacheHomeDir, "gha"), "Defines the path where the actions get cached and host workspaces created.")
	...
	rootCmd.PersistentFlags().StringVarP(&input.domain, "domain", "d", "", "Custom ngrok domain to use for OIDC server (e.g. myapp.ngrok.io)")
...
// Return locations where GHA's config can be found in order: XDG spec, .gharc in HOME directory, .gharc in invocation directory
func configLocations() []string {
	configFileName := ".gharc"
	...
	specPath, err := xdg.ConfigFile("gha/gharc")
	if err != nil {
		specPath = homePath
	}
...
func bugReport(ctx context.Context, version string) error {
	...
	report := sprintf("gha version:", version)
...
func generateManPage(cmd *cobra.Command) error {
	header := &doc.GenManHeader{
		Title:   "gha",
		Section: "1",
		Source:  fmt.Sprintf("gha %s", cmd.Version),
	}
...
func newRunCommand(ctx context.Context, input *Input) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		if runtime.GOOS == "darwin" && runtime.GOARCH == "arm64" && input.containerArchitecture == "" {
			...
			l.Warnf(" \U000026A0 You are using Apple M-series chip and you have not specified container architecture, you might encounter issues while running gha. If so, try running it with '--container-architecture linux/amd64'. \U000026A0 \n")
		}
		...
		if plan != nil {
			if len(plan.Stages) == 0 {
				plannerErr = fmt.Errorf("Could not find any stages to run. View the valid jobs with `gha --list`. Use `gha --help` to find how to filter by Job ID/Workflow/Event Name")
			}
		}
		...
		deprecationWarning := "--%s is deprecated and will be removed soon, please switch to cli: `--container-options \"%[2]s\"` or `.gharc`: `--container-options %[2]s`."
		if input.privileged {
		...
		// Check if OIDC server is running and configure environment
...
func defaultImageSurvey(actrc string) error {
	...
	switch answer {
	case "Large":
		option = "-P ubuntu-latest=milan98080/ubuntu:full-latest\n-P ubuntu-22.04=milan98080/ubuntu:full-22.04\n-P ubuntu-20.04=milan98080/ubuntu:full-20.04\n-P ubuntu-18.04=milan98080/ubuntu:full-18.04\n"
	case "Medium":
		option = "-P ubuntu-latest=milan98080/ubuntu:gha-latest\n-P ubuntu-22.04=milan98080/ubuntu:gha-22.04\n-P ubuntu-20.04=milan98080/ubuntu:gha-20.04\n-P ubuntu-18.04=milan98080/ubuntu:gha-18.04\n"
	case "Micro":
