...
func TestStepActionRemote(t *testing.T) {
	...
	for _, tt := range table {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.mocks.read {
				sarm.On("readAction", sar.Step, suffixMatcher("gha/remote-action@v1"), "", mock.Anything, mock.Anything).Return(&model.Action{}, nil)
			}
			if tt.mocks.run {
				sarm.On("runAction", sar, suffixMatcher("gha/remote-action@v1"), newRemoteAction(sar.Step.Uses)).Return(func(_ context.Context) error { return tt.runError })
...
func TestStepActionRemotePost(t *testing.T) {
	...
	for _, tt := range table {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.mocks.exec {
				cm.On("Exec", []string{"node", "/var/run/gha/actions/remote-action@v1/post.js"}, sar.env, "", "").Return(func(_ context.Context) error { return tt.err })
