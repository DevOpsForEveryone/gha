...
func execAsDocker(ctx context.Context, step actionStep, actionName, basedir, subpath string, localAction bool, entrypointType string) error {
	...
	if strings.HasPrefix(action.Runs.Image, "docker://") {
		...
		// "-dockeraction" enshures that "./", "./test " won't get converted to "gha-:latest", "gha-test-:latest" which are invalid docker image names
		image = fmt.Sprintf("%s-dockeraction:%s", regexp.MustCompile("[^a-zA-Z0-9]").ReplaceAllString(actionName, "-"), "latest")
		image = fmt.Sprintf("gha-%s", strings.TrimLeft(image, "-"))
